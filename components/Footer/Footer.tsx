        ...
        <div className="flex flex-col lg:flex-row">
          {footerLinks.map(({ heading, info, socials }: FooterLinkTypes, i) => (
            <div
              key={heading}
              className={`flex  ${
                heading == "resources" ? "lg:w-5/12" : "lg:w-2/12"
              } mx-auto`}
            >
              <div className="mb-4 space-y-4">
                <h4 className="font-semibold mb-2 text-gray-700">{heading}</h4>
                <ul className="flex flex-col space-y-4">
                  {info.map(({ name, link }: INFO) => (
                    <li key={name}>
                      <Link href={link}>
                        <a className="leading-snug text-gray-500 hover:opacity-75 font-light">
                          <span className="ml- text-lg">
                            {name}
                          </span>
                        </a>
                      </Link>
